...
CHANNELS = 1  # 单声道
RATE = 16000  # 采样率
RECORD_SECONDS = 3  # 采样时长 定义为7秒的录音
RECORD_SECONDS2 = 3 #采样指令
WAVE_OUTPUT_FILENAME = "F:/大二上课程/智能硬件应用实验/实验/myvoice.pcm"  # 采集声音文件存储路径
WAVE_OUTPUT_FILENAME2= "F:/大二上课程/智能硬件应用实验/实验/指令.pcm" #采集指令的文件存储路径
...
def word_to_voice3(text):
    result = client.synthesis(text, 'zh', 1, {
        'vol': 5, 'spd': 3, 'per': 3})
    if not isinstance(result, dict):
        with open('F:/大二上课程/智能硬件应用实验/实验/audio3.mp3', 'wb') as f:
            f.write(result)
            f.close()
    print(text)
...
def get_mic_voice_file2(p):
    ...
    frames = []
    for i in range(0, int(RATE / CHUNK * RECORD_SECONDS2)):
        data = stream.read(CHUNK)
        frames.append(data)
    print("* done recording")
    stream.stop_stream()
...
def baidu_get_song_name2(client):
    results = client.asr(get_file_content(
        WAVE_OUTPUT_FILENAME2), 'pcm', 16000, {'dev_pid': 1536, })
    if 'result' in results.keys():
        # print(results['result'])
...
def parse(keyword, num):
    ...
    for n in range(0, len(strMediaMids)):
        ...
        srcs[n] = data
    with open('F:/大二上课程/智能硬件应用实验/音乐列表/'+str(keyword)+'.json', 'w', encoding='utf-8') as f:
        json.dump(srcs, f)


def download(keyword):
    try:
        with open('F:/大二上课程/智能硬件应用实验/音乐列表/'+keyword+'.json', 'r') as f:
            data = json.load(f)
        # print(data)
        for key in data:
            time.sleep(1)
            url = data[key]['url']
            # print(url)
            if os.path.exists('F:/大二上课程/智能硬件应用实验/音乐列表/'+data[key]['歌名']+'.m4a') == False:
                print('正在下载:', data[key]['歌名'], '......')
                try:
                    urllib.request.urlretrieve(
                        url, 'F:/大二上课程/智能硬件应用实验/音乐列表/'+data[key]['歌名']+'.m4a')
                    print('下载完成！')
                    ...
                    return 'F:/大二上课程/智能硬件应用实验/实验/audio3.py'
            else:
                print("歌曲已存在")

        filename = 'F:/大二上课程/智能硬件应用实验/音乐列表/' + data[key]['歌名']+'.m4a'
        print(filename)
...
def play_mp3(music_file):
    ...
    for i, m4a in enumerate(m4a_file):
        ...
        if dot == -1:
            ...
            s = m4a[dot+1:]
        if s == 'm4a':
            if os.path.exists('F:/大二上课程/智能硬件应用实验/音乐列表/'+m4a[:dot]+'.mp3') == False:
...
if __name__ == '__main__':
    ...
    while 1:  # 循环调用麦克录音
        ...
        time.sleep(10)
        while pygame.mixer.music.get_busy() == True:
            get_mic_voice_file2(p)
            order=baidu_get_song_name2(client)
            if order=="none":
                print('''
    *****************************
    * 1.新的曲目   2.退出程序    *
