...
def get_mic_voice_file(p):
    ...
    wf.close()
    #dot = WAVE_OUTPUT_FILENAME.rfind(".")
    #wg = open("F:/大二上课程/智能硬件应用实验/实验/myvoice1.pcm", 'wb')
    # wg.close()
    # os.system("ffmpeg -i " + WAVE_OUTPUT_FILENAME
    #       + " " + "-ar 16000"+" "+"F:/大二上课程/智能硬件应用实验/实验/myvoice1.pcm")
    # ffmpeg.input(WAVE_OUTPUT_FILENAME).output("F:/大二上课程/智能硬件应用实验/实验/myvoice1.pcm",ar=16000).overwrite_output().run()
    os.system(
        "ffmpeg -y -i  F:/大二上课程/智能硬件应用实验/实验/myvoice.pcm -ar 16000  F:/大二上课程/智能硬件应用实验/实验/myvoice1.wav")
...
def baidu_get_song_name(client):
    results = client.asr(get_file_content(
        "F:/大二上课程/智能硬件应用实验/实验/myvoice1.wav"), 'wav', 16000, {'dev_pid': 1536, })
...
def download(keyword):
    global default
    try:
        with open('G:/CloudMusic/'+keyword+'.json', 'r') as f:
            data = json.load(f)
        # print(data)
        for key in data:
            time.sleep(1)
            url = data[key]['url']
            # print(url)
            if os.path.exists('G:/CloudMusic/'+data[key]['歌名']+'.m4a') == False:
                print('正在下载:', data[key]['歌名'], '......')
                try:
                    urllib.request.urlretrieve(
                        url, 'G:/CloudMusic/'+data[key]['歌名']+'.m4a')
                    print('下载完成！')
        ...
        filename = 'G:/CloudMusic/' + data[key]['歌名']+'.m4a'
        print(filename)
...
def play_mp3(music_file):
    ...
    for i, m4a in enumerate(m4a_file):
        ...
        if dot == -1:
            ...
            s = m4a[dot+1:]
        if s == 'm4a':
            if os.path.exists('G:/CloudMusic/'+temp[:dot1]+'.mp3') == False:
                if temp != m4a:
                    print(m4a)
